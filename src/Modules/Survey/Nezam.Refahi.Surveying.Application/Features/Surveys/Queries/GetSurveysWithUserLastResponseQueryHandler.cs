...
public class GetSurveysWithUserLastResponseQueryHandler : IRequestHandler<GetSurveysWithUserLastResponseQuery, ApplicationResult<SurveysWithUserLastResponseResponse>>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Always include Questions and Responses for mapping (they're needed for counts and user response logic)
            var (surveys, totalCount) = await _surveyRepository.GetSurveysWithPaginationAsync(
                ...
                includeQuestions: true, // Always include for TotalQuestions, RequiredQuestions counts
                includeResponses: true, // Always include for ResponseCount, UniqueParticipantCount, and user response logic
                cancellationToken: cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            return ApplicationResult<SurveysWithUserLastResponseResponse>.Failure(ex,"خطا در دریافت لیست نظرسنجی‌ها");
        }
    }
    ...
    private SurveyDto MapToSurveyDto(Survey survey, GetSurveysWithUserLastResponseQuery request, Dictionary<Guid, Response> userLastResponses, Guid? memberId)
    {
        ...
        if (survey.StartAt.HasValue && survey.EndAt.HasValue)
        {
            ...
            if (survey.IsAcceptingResponses() && survey.EndAt > DateTimeOffset.UtcNow)
            {
