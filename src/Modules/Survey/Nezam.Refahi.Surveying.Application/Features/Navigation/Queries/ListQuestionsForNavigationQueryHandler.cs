...
namespace Nezam.Refahi.Surveying.Application.Features.Navigation.Queries;
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error occurred while getting questions navigation for response {ResponseId} in survey {SurveyId}", 
                request.ResponseId, request.SurveyId);
            return ApplicationResult<QuestionsNavigationResponse>.Failure(ex, "خطا در دریافت سوالات نظرسنجی برای ناوبری");
        }
    }
