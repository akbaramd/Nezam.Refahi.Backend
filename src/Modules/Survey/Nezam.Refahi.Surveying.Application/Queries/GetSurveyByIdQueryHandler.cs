...
public class GetSurveyByIdQueryHandler : IRequestHandler<GetSurveyByIdQuery, ApplicationResult<SurveyDto>>
{
            ...
            if (!string.IsNullOrWhiteSpace(request.UserNationalNumber))
            {
                ...
                if (memberInfo != null)
                {
                    ...
                    _logger.LogInformation("Found {ResponseCount} responses for member {MemberId} in survey {SurveyId}", 
                        userResponses.Count, memberInfo.Id, request.SurveyId);
                    ...
                    var eligibilityResult = await CheckComprehensiveEligibility(survey, memberInfo.Id, latestResponse, cancellationToken);
                    ...
                    surveyDto.UserEligibility = await GetDetailedEligibilityInfo(survey, memberInfo.Id, latestResponse, cancellationToken);
                }
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 1. Check survey state
        eligibility.IsSurveyActive = survey.State == SurveyState.Active;
