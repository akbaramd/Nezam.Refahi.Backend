...
public interface ITokenService
{
  ...
  /// <returns>Tuple of (accessToken, expiryMinutes, jwtId)</returns>
  Task<(string AccessToken, int ExpiryMinutes, string JwtId)> GenerateAccessTokenAsync(
    User user,  
  ...
  /// Returns validation result with token details when valid
  /// </summary>
  /// <param name="token">The token to validate</param>
  /// <returns>Tuple of (isValid, jwtId, userId) - jwtId and userId are null when validation fails</returns>
  (bool IsValid, string? JwtId, Guid? UserId) ValidateAccessToken(string token);
  ...
  /// </summary>
  /// <param name="userId">UserDetail ID for the token</param>
  /// <param name="deviceFingerprint">Device fingerprint for binding</param>
  /// <param name="ipAddress">IP address for binding</param>
  /// <param name="userAgent">UserDetail agent for binding</param>
  ...
  /// </summary>
  /// <param name="userId">UserDetail ID to revoke tokens for</param>
  Task RevokeAllUserRefreshTokensAsync(Guid userId);

  /// <summary>
  /// Revokes refresh tokens for a specific device
  /// </summary>
  /// <param name="userId">UserDetail ID</param>
