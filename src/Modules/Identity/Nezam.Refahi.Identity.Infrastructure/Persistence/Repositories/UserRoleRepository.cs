...
public class UserRoleRepository : EfRepository<IdentityDbContext, UserRole, Guid>, IUserRoleRepository
{
    ...
    public async Task<UserRole?> GetActiveByUserAndRoleAsync(Guid userId, Guid roleId, CancellationToken cancellationToken = default)
    {
        return await _dbContext.UserRoles
            .Include(ur => ur.Role)
            .Include(ur => ur.User)
            .FirstOrDefaultAsync(ur => ur.UserId == userId && ur.RoleId == roleId && ur.IsActive && (ur.ExpiresAt == null || ur.ExpiresAt > DateTime.UtcNow), cancellationToken);
    }
    ...
    {
        return await _dbContext.UserRoles
            .CountAsync(ur => ur.UserId == userId && ur.IsActive && (ur.ExpiresAt == null || ur.ExpiresAt > DateTime.UtcNow), cancellationToken);
    }

    public async Task<int> GetActiveCountByRoleIdAsync(Guid roleId, CancellationToken cancellationToken = default)
    {
        return await _dbContext.UserRoles
            .CountAsync(ur => ur.RoleId == roleId && ur.IsActive && (ur.ExpiresAt == null || ur.ExpiresAt > DateTime.UtcNow), cancellationToken);
    }
    ...
    {
        return await _dbContext.UserRoles
            .AnyAsync(ur => ur.UserId == userId && ur.RoleId == roleId && ur.IsActive && (ur.ExpiresAt == null || ur.ExpiresAt > DateTime.UtcNow), cancellationToken);
    }
