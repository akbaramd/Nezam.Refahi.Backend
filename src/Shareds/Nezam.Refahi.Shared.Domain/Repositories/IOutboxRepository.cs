...
public interface IOutboxRepository : IRepository<OutboxMessage, Guid>
{
    ...
    /// <summary>
    /// Marks a message as processed with optimistic concurrency control
    /// </summary>
    Task<bool> MarkAsProcessedWithConcurrencyAsync(Guid messageId, int expectedRetryCount, CancellationToken cancellationToken = default);

    /// <summary>
    /// Marks a message as failed with retry scheduling and concurrency control
    /// </summary>
    Task<bool> MarkAsFailedWithConcurrencyAsync(Guid messageId, string error, int expectedRetryCount, CancellationToken cancellationToken = default);
